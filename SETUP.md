...
## 7. Chạy song song bản SaliGuard cũ để đối chiếu (dành cho demo/giám khảo)

Nút **"Phiên bản"** trên thanh trên cùng của ForestGump mở ra link sang bản SaliGuard gốc (giao
diện mặn hoá cũ, tag git `saliguard-legacy`), chạy độc lập ở cổng **5174** qua `git worktree` —
không đụng tới code/build của ForestGump:

```bash
# Chỉ cần làm 1 lần
git worktree add ../tesssst-legacy saliguard-legacy
...
Sau đó mở ForestGump ở `:5173`, bấm nút "Phiên bản" → "SaliGuard (cũ)" để mở tab mới sang `:5174`.
