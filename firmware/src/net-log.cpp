...
void netLogBegin() {
	...
	ArduinoOTA.setHostname("saliguard-" SECRET_STATION_ID);
	ArduinoOTA.onStart([]() { Serial.println(F("[OTA] Bắt đầu nạp qua WiFi...")); });
	ArduinoOTA.onEnd([]()   { Serial.println(F("[OTA] Xong, khởi động lại.")); });
	ArduinoOTA.begin();
	Serial.print(F("[NETLOG] OTA sẵn sàng tại IP trên (hostname saliguard-" SECRET_STATION_ID ")."));
	Serial.println();
}
...
void netLogLoop() {
	...
	if (telnetServer.hasClient()) {
		if (telnetClient && telnetClient.connected()) {
			...
			telnetClient.println(F("=== SaliGuard log (Telnet) - station " SECRET_STATION_ID " ==="));
