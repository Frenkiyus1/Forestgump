# ForestGump Dashboard — Component Clone Design

**Date:** 2026-06-27
**Block:** `/dashboard` (SvelteKit, Svelte 5, Tailwind v4)
**Goal:** Reproduce the reference admin-dashboard layout (the "OrangeFarm" CRM mockup) as the ForestGump
...
- Rebrand every widget to ForestGump domain (stations, salinity, alerts, forecasts).
...
- **Alert dots/badges:** always green / yellow / red per the ForestGump threshold table — independent of
  the orange accent.
- **Chart:** uPlot (CLAUDE.md mandate). `uplot` to be added to `dashboard` deps.

## 3. Widget → ForestGump mapping

| Reference widget         | ForestGump widget                                   | Data source                          |
|--------------------------|----------------------------------------------------|--------------------------------------|
| Sidebar (OrangeFarm)     | Sidebar (ForestGump): Dashboard, Stations, Map, Alerts, History, Reports, Export | static |
