<script lang="ts">
	...
	const anchorEntry = $derived(
		data.forecastEntries.find((e) => e.location.name === 'Phường Điện Biên Phủ') ?? null
	);
</script>

<svelte:head><title>Phường Điện Biên Phủ — ForestGump</title></svelte:head>
...
<AppShell>
	<nav class="mb-3 flex items-center gap-1.5 text-sm text-gray-400">
		...
		<span class="font-semibold text-gray-900">Phường Điện Biên Phủ</span>
	</nav>
	...
	<p class="mt-2 text-[15px] text-gray-500">
		Bản đồ chi tiết khu vực Phường Điện Biên Phủ — huyện Điện Biên Phủ.
	</p>

	<div class={clsx(CARD, 'relative mt-6 overflow-hidden p-0')}>
		<img
			src="/dien-bien-phu/map.png"
			alt="Bản đồ huyện Điện Biên Phủ"
