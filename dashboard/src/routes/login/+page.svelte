...
<svelte:head><title>Đăng nhập — ForestGump</title></svelte:head>
	...
	<section class="flex flex-col justify-center px-8 py-12 sm:px-16">
		<div class="mx-auto w-full max-w-sm">
			...
			<p class="mt-1 text-sm text-slate-500">
				Theo dõi cảnh báo thời tiết Điện Biên theo thời gian thực.
			</p>
			...
			<form class="mt-8 flex flex-col gap-4" onsubmit={onSubmit}>
				<label class="flex flex-col gap-1 text-sm font-medium text-slate-700">
					...
					<input
						type="email"
						bind:value={email}
						placeholder="admin@forestgump.vn"
			...
			<p class="mt-6 text-xs text-slate-400">Demo: admin@forestgump.vn / forestgump123</p>
		</div>
	</section>
</div>
